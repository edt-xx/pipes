...
pub fn pipeType(comptime T:type) type {
    return struct {
        ...
        pub fn exactdiv(s:*Stage,d:Data) callconv(.Async) !void { return myStages.exactdiv(s,d); }
        pub fn console(s:*Stage) callconv(.Async) !void  { return myStages.console(s); }
        pub fn fanin(s:*Stage) callconv(.Async) !void { return myStages.fanin(s); }
        pub fn gen(s:*Stage,d:Data) callconv(.Async) !void { return myStages.gen(s,d); }
...
const myStages = struct {
    ...
    fn exactdiv(self:anytype, d:anytype) !void {
        ...
        while (true) {
            ...
            if (i%d == 0)
                ...
                self.selectOutput(1) catch |err| {if (err != error.noOutStream) return err; };
            if (debugStages) std.log.info("div out {}_{s} {*}",.{self.i, self.name, self.outC});
            _ = self.output(i) catch |err| {if (err != error.noOutStream) return err; };            
            _ = try self.readto();
        }
    }
    ...
    fn fanin(self:anytype) !void {
        ...
        if (debugStart) std.log.info("start {}_{s}",.{self.i, self.name});
        while (true) {
            _ = self.selectAnyInput() catch |err| {if (err == error.endOfStream) continue else return err; };
                                ...
                                inline for (elem) |arg, k| {                        
                                    switch (@typeInfo(@TypeOf(arg))) {
                                        ...
                                        .Pointer => {
                                            ...
                                            comptime { tuple[i][1][k+1] = @field(x,arg);  }
                                        },
        ...
        pub fn setup(self:*thePipe) thePipe {
            ...
            for (p) |item, ii| {
                ...
                if (p[ii].label) |lbl| {
                    ...
                    map.put(lbl,k.*) catch {unreachable;};
                } else if (stg) {
                        var k = Conn{};
                        k.set(p,ii,1,ii,1);
                        map.put(lbl,k) catch {unreachable;};
                    ...
                    if (c.dst.state == .start and c.dst.commit == commit) {
                        ...
                        inline while (j < p.len) : (j += 1) {
                            if (@TypeOf(tuple[j][0]) != void) {     // prevent inline from generating void calls
                                if (c.dst.i == j)
                                    _ = @asyncCall(allocator.alignedAlloc(u8,16,@frameSize(tuple[j][0])) catch {unreachable;}, &self.p[j].rc, tuple[j][0], tuple[j][1] );
                            }
                        }
                        continue :running;
                    }
